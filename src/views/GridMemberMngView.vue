...
import { Gender, GridMemberState } from '@/common/enums'
import { gridMembers } from '@/common/testData'
import type { UserDTO } from '@/api/entities/user'
import { type AreaInfo, findAreaById, formatGender, formatGridMemberState, formatUseTime } from '@/util'
import { useUserStore } from '@/stores'
import { getGridMembers, selectGridMembers, setUserStatus } from '@/api'
...
export type SearchGridMemberCondition = {
  name?: string,
  gender?: Gender,
  grid_id?: string,
...
// search box data
const searchCondition = ref<SearchGridMemberCondition>({
  name: '',
  gender: undefined,
  grid_id: '',
  state: undefined,
...
const handleQuery = async () => {
  try {
    showData.value = await selectGridMembers({
      ...
      <el-form :inline="true" :model="searchCondition" class="demo-form-inline">
        ...
        <el-form-item label="状态">
          <el-select v-model="searchCondition.state" clearable style="width: 100px">
            <el-option
              v-for="option in Object.values(GridMemberState).filter(value => typeof value === 'number')"
              :key="option"
              :label="formatGridMemberState(option as GridMemberState)"
              :value="option"
            />
          </el-select>
        </el-form-item>
    ...
    <div class="table">
      <el-table :data="showData" style="width: 100%" max-height="680px">
        ...
        <el-table-column prop="state" label="状态">
          <template #default="{ row }">
            <el-tag>{{ formatGridMemberState(row.state) }}</el-tag>
          </template>
        </el-table-column>
